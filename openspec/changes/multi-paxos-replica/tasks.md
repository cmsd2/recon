...
- [ ] 1.1 Same edition as change 1 — **van Renesse, R. and Altinbuken, D. (2015), *ACM Computing
...
- [ ] 1.2 Record in the module that the fourth liveness violation implemented in group 5 is Liu,
...
- [ ] 3.1 `Command<V>` as the source's `c = ⟨κ, cid, op⟩` — the appending process, a request
...
- [ ] 3.2 State the request identifier's scope in the module: **this incarnation**, volatile, and
...
- [ ] 4.1 `MultiPaxosReplica<V, L>` holding `Child<MultiPaxosSynod<Command<V>, L>>` and nothing
...
- [ ] 4.2 `slot_in`, `slot_out`, `requests`, `proposals`, `decisions`, with R1–R5 stated. Verify
      `decisions` is append-only, as the page has it and says why
- [ ] 4.3 `propose()`: move requests into unused slots within the window, and hand each to the child
      as `Cmd::Propose`. Quote the function. Verify a request appended while the process is passive
      is proposed once a leader exists
- [ ] 4.4 `perform()`: the already-decided check, then extend the sequence. Quote the function and
...
- [ ] 4.5 Verify `Position` and `Slot` diverge exactly by the duplicate count: assert positions are
      contiguous from `Position::START` while `slot_out` has run further. This is the mix-up the
      types cannot catch
- [ ] 4.6 Decisions arriving out of order and more than once: hold them, extend the sequence only in
...
- [ ] 4.7 A request whose slot was decided for a different command goes back into `requests` and is
...
- [ ] 5.1 One periodic timer, compared against its registered `TimerId` before acting. Verify a
      process registering it does not act on the child's expiries
- [ ] 5.2 Liu et al.'s fourth violation: a proposal outstanding beyond a threshold is proposed
...
- [ ] 5.3 Verify re-proposal is harmless where the slot is not stalled: drive re-proposals against a
      healthy run and assert the sequence is unchanged and nothing is ordered twice
- [ ] 5.4 The `WINDOW` guard (R5): verify proposals stop a configured distance ahead of `slot_out`
...
- [ ] 5.5 Choose the re-proposal threshold against `detect_after` and the delivery bound — the
...
- [ ] 6.1 Implement `TotalOrderLog<V>`: `append`, `read` from a `Position`, and a total `classify`.
      Verify the port's `compile_fail` doctest still rejects a non-log
- [ ] 6.2 `read` serves the applied prefix from this process's own copy, as the port's header
      requires. Verify a read at a process whose slot is undecided returns the shorter prefix rather
      than waiting
- [ ] 6.3 Register the implementation in `tests/total_order_log.rs` so every shared property runs
      against it. Verify all of them pass
- [ ] 6.4 **Check each shared property is non-vacuous against this implementation**, rather than
...
- [ ] 6.5 Verify the crash property: a minority crashes and is **not** restarted, and the survivors
      keep ordering. Assert the crash landed before the step that depends on it

## 7. Scope, space and the boundary

- [ ] 7.1 A session ending from the child is propagated, not absorbed. Verify it reaches the layer
      above and that the module states it bridges nothing
- [ ] 7.2 State the space bound in the module: unbounded, a transcription, and §4.2 is what bounds
...
- [ ] 7.3 Verify the send rate is flat once the work is done — `tests/common::assert_send_rate_flat!`
      — and say what keeps it flat, since the re-proposal timer is the thing that could make it not
- [ ] 7.4 Verify the module documents the crash-stop boundary and what it adds to the core's: a
      returning replica would report a shortened sequence, which the ordering requirement forbids

## 8. What this dates

- [ ] 8.1 `README.md`: the protocol table, the suite table and counts, and the specification tree.
      Verify the counts against `cargo test --workspace`
- [ ] 8.2 `README.md`'s roadmap item 5 — advance it from "the Synod core is built" to "there is a
      log", and say that bounding is what remains before the real-world set
- [ ] 8.3 `README.md`'s real-world set section: the obligation still unmet is resource use, and this
      change does not meet it. Verify the wording does not now imply it does
- [ ] 8.4 `./scripts/check.sh` passes in full
- [ ] 8.5 `./scripts/check-safety-tests.sh` and `./scripts/check-durability-tests.sh` both pass. The
